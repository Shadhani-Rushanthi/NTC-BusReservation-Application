...
export const busOperatorRegister = async (req, res, next ) => {
    try {
        ...
        error.code === 11000 ? res.status(409).json("Bus Operator email already exists.") : next(error); 
    }
}
...
export const login = async (req, res, next)=>{
    try {
        ...
        const token = jwt.sign({ id:user._id, role: user.roles }, process.env.JWT,{ expiresIn: '60s' });
